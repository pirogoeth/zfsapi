...
from zfsapi.util import progressbar, SpecialPopen
...
class ZFSConnection:
    ...
    def pools(self):
        ...
        return self._get_poolset()

    def _get_poolset(self):

        if self._dirty:
            res = subprocess.check_output(self.command + 
                    ["get", "-Hpr", "-o", "name,value", "creation"])
            self._poolset.parse_zfs_r_output(res)
            self._dirty = False
        return self._poolset

    def load_properties(self, poolset):

        for dataset in poolset.walk():
            res = subprocess.check_output(self.command + 
                    ["get", "-Hp", "-o", "property,value", dataset.name])
            dataset._parse_zfs_properties(res)
            self._dirty = False
    ...
    def snapshot_recursively(self, name, snapshotname):
        ...
        subprocess.check_call(self.command + ["snapshot", "-r", "%s@%s" 
            % (name, snapshotname)])
        self._dirty = True
    ...
    def send(self, name, opts = None, bufsize = -1, compression = False):
        ...
        p = SpecialPopen(cmd, stdin = file(os.devnull), stdout = subprocess.PIPE, 
                bufsize = bufsize)
        return p
    ...
    def transfer(self, dst_conn, s, d, fromsnapshot = None, showprogress = False, 
            bufsize = -1, send_opts = None, receive_opts = None, ratelimit = -1, 
            compression = False):
        ...
        sndprg = self.send(s, opts=[] + fromsnapshot + send_opts, bufsize = bufsize, 
                compression = compression)
        sndprg_supervisor = Thread(target = lambda: 
                queue_of_killables.put((sndprg, sndprg.wait())))
        sndprg_supervisor.start()

        if showprogress:
            try:
                barprg = progressbar(pipe = sndprg.stdout, bufsize = bufsize, 
                        ratelimit = ratelimit)
                barprg_supervisor = Thread(target = 
                        lambda: queue_of_killables.put((barprg, barprg.wait())))
        ...
        try:
            rcvprg = dst_conn.receive(d, pipe = barprg.stdout, opts=["-Fu"] + receive_opts,
                    bufsize = bufsize, compression = compression)
            rcvprg_supervisor = Thread(target =
                    lambda: queue_of_killables.put((rcvprg, rcvprg.wait())))
